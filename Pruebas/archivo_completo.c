...
ESTADO Transicion(ESTADO estado, int simbolo) {
    static ESTADO TT[15][13] = {  
        ...
        //Prototipo inicial de TT 

        /*cambio el numero de estados a 15 (filas, originalmente estaba en 12) como se menciona en la descripcion 
    ...
    int columna = ObtenerColumna(simbolo); //se obtiene la columna segun el simbolo de tipo char, isdigit y isalpha apesar de que dice que ingresan int se espera un char, investigar si tienen dudas
    ESTADO nuevoEstado = TT[estado][columna]; // hacer la transicion segun la matriz 2x2
    return nuevoEstado;
}
...
TOKEN Scanner(void) {
    ...
    //ponemos en cero correctamente el buffer
...
int main(void) {
    ...
    const char *OUTPUT_TOKEN[] = {"INICIO", "FIN", "LEER", "ESCRIBIR", "ID", "CONSTANTE", "PARENIZQUIERDO",
    "PARENDERECHO", "PUNTOYCOMA", "COMA", "ASIGNACION", "SUMA", "RESTA", "FDT"};

    TOKEN t;
    while ((t = Scanner()) != 13) {
        
        if(t!=99){ //cuando Scanner devuelve un estado, el output remplaza el digito correspondiente por el nombre de token. Ej: 4 -> abc seria ID -> abc
            printf(" \t%s\t-> %s  \n", Buffer(),OUTPUT_TOKEN[t] );
        //printf("\t %s -> %s \n", OUTPUT_TOKEN[t], Buffer());
        };
